...
public class Dev_Launcher : MonoBehaviourPunCallbacks
{
    ...
    {
        Debug.Log($"{nameof(Dev_Launcher)}:{nameof(OnJoinRandomFailed)} was called by PUN. No random rooms available, so one will be created.\nCalling {nameof(PhotonNetwork)}.{nameof(PhotonNetwork.CreateRoom)}");
        PhotonNetwork.CreateRoom(null, new RoomOptions { MaxPlayers = maxPlayersPerRoom });
    }

    public override void OnJoinedRoom()
    {
        Debug.Log($"{nameof(Dev_Launcher)}:{nameof(OnJoinedRoom)} was called by PUN. Client is now onnected to a room");
        controlPanel.SetActive(false);

        if (PhotonNetwork.CurrentRoom.PlayerCount == 1)
        {
            Debug.Log("The 'Room for 1' scene will be loaded");
